...
export function mergeOwnerMediaPreviews(
  ...
  const incomingById = new Map(incoming.map((preview) => [preview.item.mediaAsset.id, preview]));
  return mergeBoundOwnerMediaItems(
    ...
    item,
    signedUrl: incomingById.get(item.mediaAsset.id)?.signedUrl ?? currentById.get(item.mediaAsset.id)?.signedUrl ?? null,
  }));
}
