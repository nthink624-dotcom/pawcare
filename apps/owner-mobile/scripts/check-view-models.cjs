...
assert.equal(firstRow.statusLabel, "?•ì •");
...
assert.equal(getAppointmentStatusLabel("pending"), "?•ì •");
assert.equal(getAppointmentStatusLabel("confirmed"), "?•ì •");
assert.equal(getAppointmentStatusLabel("in_progress"), "ë¯¸ìš©ì¤?);
assert.equal(getAppointmentStatusLabel("almost_done"), "?½ì—… ì¤€ë¹?);
assert.equal(getAppointmentStatusLabel("completed"), "?„ë£Œ");
assert.equal(getAppointmentStatusLabel("cancelled"), "ì·¨ì†Œ");
assert.equal(getAppointmentStatusLabel("rejected"), "ë¯¸ìŠ¹??);
assert.equal(getAppointmentStatusLabel("noshow"), "?¸ì‡¼");
