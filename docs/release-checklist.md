...
- 상태는 카드 전체 색상이나 dot가 아니라 왼쪽 감싸는 인디케이터로 표시
- schedule/calendar 감싸는 인디케이터는 부모 카드/셀의 공통 `pm-wrap-indicator` class 형태 유지
- 부모 카드/셀은 왼쪽 edge에 `box-shadow: inset 1px 0 0 var(--pm-wrap-indicator-color)`를 함께 사용
- `pm-wrap-indicator::before`는 4px 너비, `top:1px`, `bottom:1px`, `border-radius:999px` 형태 유지
- 감싸는 인디케이터가 gradient fade로 면이 사라져 보이지 않는지 확인
- 부모 카드/셀은 `overflow-hidden`과 8px radius 유지
- 감싸는 인디케이터가 캘린더 선택 카드의 왼쪽 면처럼 보이고, 전체 테두리까지 색칠되지 않는지 확인
- 감싸는 인디케이터가 자식 span이나 별도 border-left로 구현되지 않았는지 확인
- 감싸는 인디케이터 색상이 스케줄, 캘린더, 주간 근무표에서 동일한지 확인
- 점 인디케이터와 감싸는 인디케이터 모두 `src/components/owner-web/status-indicators.ts`의 공통 색상 토큰을 사용하는지 확인
- 상태 rail 색상은 확정/진행 teal `#2f9b8a`, 대기/변경 amber `#d99a2b`, 취소/거절 burgundy `#8f2438`, 완료 slate `#64748b` 기준 유지
...
- 날짜 이동은 주간 근무표 헤더/툴바 안에서 작고 조용하게 표시
- 왼쪽 chevron, 가운데 plain date text, 오른쪽 chevron 패턴 유지
- 날짜 이동을 큰 별도 bordered date row로 분리하지 않음
...
- 모든 주간 근무표 cell은 흰 배경 유지
- 모든 주간 근무표 cell은 왼쪽 감싸는 인디케이터로만 상태 표시
- 근무 cell은 teal `#2f9b8a` rail
- 휴무 cell은 burgundy `#8f2438` rail
- 연차/반차/승인대기 cell은 amber `#d99a2b` rail
- 감싸는 인디케이터는 부모 카드/셀의 공통 `pm-wrap-indicator` class 형태 유지
- 부모 카드/셀은 왼쪽 edge에 `box-shadow: inset 1px 0 0 var(--pm-wrap-indicator-color)`를 함께 사용
- `pm-wrap-indicator::before`는 4px 너비, `top:1px`, `bottom:1px`, `border-radius:999px` 형태 유지
- 감싸는 인디케이터가 gradient fade로 면이 사라져 보이지 않는지 확인
- 부모 카드/셀은 `overflow-hidden`과 8px radius 유지
- 감싸는 인디케이터가 캘린더 선택 카드의 왼쪽 면처럼 보이고, 전체 테두리까지 색칠되지 않는지 확인
- 감싸는 인디케이터가 자식 span이나 별도 border-left로 구현되지 않았는지 확인
- 감싸는 인디케이터 색상이 스케줄/캘린더와 동일한지 확인
- cell 안에는 상태 점을 넣지 않고 텍스트를 가운데 정렬
- 기본 근무 설정은 기본 접힘 상태
