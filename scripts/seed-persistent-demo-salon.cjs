...
(async () => {
  ...
  fail(await supabase.from("appointments").insert(appointmentRows), "데모 예약 저장");
  console.log(JSON.stringify({ shopId, staff: staff.length, customers: customers.length, appointments: appointmentRows.length }));
