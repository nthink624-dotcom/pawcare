...
try {
  ...
  Invoke-PgTool "psql.exe" ($psql + @("-f", (Join-Path $repoRoot "supabase\migrations\20260829023403_harden_atomic_owner_signup.sql")))
  Invoke-PgTool "psql.exe" ($psql + @("-f", (Join-Path $repoRoot "tests\sql\atomic-signup-v5-forward-assertions.sql")))

  $prepareSql = @"
insert into auth.users(id,email)
select ('20000000-0000-0000-0000-' || lpad(i::text, 12, '0'))::uuid, 'owner' || i || '@example.invalid' from generate_series(1,20) i;
...
"@
  ...
  # An unexpected private-schema object must abort the transaction before any
  # wrapper or implementation function is removed.
  Invoke-PgTool "psql.exe" ($psql + @("-c", "create domain pm_signup_private.unexpected_domain as text;"))
  Invoke-PgToolExpectFailure "psql.exe" ($psql + @("-f", (Join-Path $repoRoot "supabase\rollback\20260829023403_harden_atomic_owner_signup.rollback.sql")))
  Invoke-PgTool "psql.exe" ($psql + @("-f", (Join-Path $repoRoot "tests\sql\atomic-signup-v5-forward-assertions.sql")))
  Invoke-PgTool "psql.exe" ($psql + @("-c", "do `$`$ begin if to_regtype('pm_signup_private.unexpected_domain') is null then raise exception 'PM_SIGNUP_V5_ROLLBACK_PARTIALLY_MUTATED_SCHEMA'; end if; end `$`$; drop domain pm_signup_private.unexpected_domain;"))

  # A rollback must stop while a request may be between Auth creation and the
  # atomic product write. Completed product rows are safe and must be retained.
  Invoke-PgTool "psql.exe" ($psql + @("-c", "insert into public.signup_idempotency_requests(signup_request_id,payload_hash,status) values('b0000000-0000-0000-0000-000000000001',repeat('b',64),'claimed');"))
  Invoke-PgToolExpectFailure "psql.exe" ($psql + @("-f", (Join-Path $repoRoot "supabase\rollback\20260829023403_harden_atomic_owner_signup.rollback.sql")))
  Invoke-PgTool "psql.exe" ($psql + @("-f", (Join-Path $repoRoot "tests\sql\atomic-signup-v5-forward-assertions.sql")))
  Invoke-PgTool "psql.exe" ($psql + @("-c", "delete from public.signup_idempotency_requests where signup_request_id='b0000000-0000-0000-0000-000000000001';"))
  Invoke-PgTool "psql.exe" ($psql + @("-f", (Join-Path $repoRoot "supabase\rollback\20260829023403_harden_atomic_owner_signup.rollback.sql")))
  Invoke-PgTool "psql.exe" ($psql + @("-f", (Join-Path $repoRoot "tests\sql\atomic-signup-v5-rollback-assertions.sql")))
  Invoke-PgTool "psql.exe" ($psql + @("-c", "do `$`$ begin if (select count(*) from public.owner_profiles)<>20 or (select count(*) from public.owner_subscriptions)<>20 or (select count(*) from public.owner_trial_identity_claims)<>1 then raise exception 'PM_SIGNUP_V5_ROLLBACK_CHANGED_COMPLETED_DATA'; end if; end `$`$;"))
  Invoke-PgTool "psql.exe" ($psql + @("-f", (Join-Path $repoRoot "supabase\migrations\20260829023403_harden_atomic_owner_signup.sql")))
  Invoke-PgTool "psql.exe" ($psql + @("-f", (Join-Path $repoRoot "tests\sql\atomic-signup-v5-forward-assertions.sql")))
...
insert into auth.users(id,email) values('a1000000-0000-0000-0000-000000000001','rotation@example.invalid');
insert into public.signup_idempotency_requests(signup_request_id,payload_hash,status,auth_user_id)
...
"@
  ...
  $failureSql = @"
insert into auth.users(id,email) values ('50000000-0000-0000-0000-000000000001','retry@example.invalid');
insert into public.signup_idempotency_requests(signup_request_id,payload_hash,status,auth_user_id)
...
"@
  ...
  Write-Output "REUSED_PHONE_SINGLE_TRIAL_POSTGRES_PASS accounts=20 trial_winners=1 rotation_regain=0 signup_credits=0 product_snapshot=immutable failed_signup_claims=0 retry_trial=14 direct_claim=denied backfill=earliest v5_roundtrip=pass rollback=pass"
}
finally {
