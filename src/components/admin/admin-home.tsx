...
import { buildAdminKanbanItems, type AdminKanbanColumn, type AdminKanbanItem } from "@/components/admin/admin-kanban";
...
export default function AdminHome({ sessionLoginId }: { sessionLoginId: string }) {
  ...
  const kanbanItems = activated ? buildAdminKanbanItems(requests, marketing) : [];
  ...
  return <main className="min-h-screen bg-[#f4f7fb] px-5 py-6 text-[#172033] lg:px-8 lg:py-8">
    <div className="mx-auto w-full max-w-[1440px]">
      ...
      <section className="mt-4" aria-labelledby="kanban-title">
        <div className="flex items-center justify-between gap-3"><h2 id="kanban-title" className={`text-[#0f172a] ${ADMIN_TYPOGRAPHY.sectionTitle}`}>업무 상태 칸반</h2><span className="text-[12px] text-[#64748b]">각 업무는 한 상태에만 표시됩니다</span></div>
        <div className="mt-3 grid gap-3 lg:grid-cols-3">
          <KanbanColumn title="조치 필요" column="action" tone="amber" items={kanbanItems} activated={activated} />
          <KanbanColumn title="검수 필요" column="review" tone="active" items={kanbanItems} activated={activated} />
          <KanbanColumn title="완료" column="complete" tone="confirmed" items={kanbanItems} activated={activated} />
        </div>
      </section>
...
function KanbanColumn({ title, column, tone, items, activated }: { title: string; column: AdminKanbanColumn; tone: StatusIndicatorTone; items: AdminKanbanItem[]; activated: boolean }) { const visible = items.filter((item) => item.column === column); return <section className="rounded-[10px] border border-[#dce5f0] bg-[#f8fafc] p-3" aria-label={title}><header className="flex min-h-11 items-center justify-between px-1"><h3 className="text-[14px] font-medium text-[#0f172a]">{title}</h3><span className="inline-flex items-center gap-2 text-[12px] text-[#64748b]"><span className={getDotIndicatorClass(tone)} />{visible.length}</span></header><div className="grid gap-2">{visible.map((item) => <article key={item.id} data-kanban-item-id={item.id} className={`${getWrapIndicatorClass(item.tone)} px-3 py-3`}><p className="line-clamp-2 text-[14px] font-medium text-[#0f172a]">{item.title}</p><p className="mt-1 truncate text-[12px] text-[#64748b]">{item.meta}</p></article>)}{visible.length === 0 ? <p className="rounded-[8px] border border-dashed border-[#dbe2ea] bg-white px-3 py-4 text-center text-[12px] text-[#94a3b8]">{activated ? "해당 업무가 없습니다" : "상세를 열면 표시됩니다"}</p> : null}</div></section>; }
