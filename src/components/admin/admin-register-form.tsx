...
export default function AdminRegisterForm() {
  ...
  async function handleSubmit() {
    ...
    try {
      await fetchApiJson<{ success: true }>("/api/admin/auth/register", {
        method: "POST",
      ...
      setMessage(error instanceof Error ? error.message : "관리자 등록에 실패했습니다.");
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <main className="mx-auto min-h-screen w-full max-w-[430px] bg-white px-6 py-10 text-[#171411]">
      <div className="rounded-[24px] border border-[#e8dfd3] bg-white px-5 py-6 shadow-[0_18px_40px_rgba(23,20,17,0.06)]">
        ...
        <div className="mt-5 text-center">
          <p className="text-[13px] font-semibold tracking-[0.04em] text-[#1f6b5b]">최초 1회 설정</p>
        ...
        <div className="mt-7 space-y-3">
          ...
          <div className="space-y-2">
            ...
            <p className="px-1 text-[12px] leading-5 text-[#8a8075]">
              `.env.local`의 <span className="font-semibold text-[#5f554a]">ADMIN_SETUP_KEY=</span> 뒤에 있는 값만 붙여넣어 주세요.
            </p>
          </div>
        </div>
