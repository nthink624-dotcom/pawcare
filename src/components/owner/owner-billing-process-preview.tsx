...
function PreviewSection({
  ...
  return (
    <section className="rounded-[12px] border border-[var(--pm-ui-border)] bg-white p-5 shadow-none">
      <p className="text-[12px] font-medium tracking-[-0.02em] text-[var(--acc)]">화면 {step}</p>
      <h2 className="mt-1 text-[24px] font-semibold tracking-[-0.04em] text-[var(--ink)]">{title}</h2>
      <p className="mt-2 text-[13px] leading-[1.55] text-[var(--mid)]">{description}</p>
...
function ProcessCard({
  ...
  return (
    <div className="rounded-[10px] border border-[var(--pm-ui-border)] bg-white p-4">
      <p className="text-[15px] font-medium tracking-[-0.02em] text-[var(--ink)]">{title}</p>
      <p className="mt-1 text-[13px] leading-[1.55] text-[var(--mid)]">{description}</p>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-4 py-3">
      <p className="text-[14px] text-[var(--mid)]">{label}</p>
      <p className="text-right text-[15px] tracking-[-0.02em] text-[var(--ink)]">{value}</p>
    </div>
  );
}
...
function ActionButton({
  ...
  return (
    <button
      type="button"
      className={`flex h-[52px] items-center justify-center rounded-[8px] px-4 text-[15px] tracking-[-0.03em] ${
        primary
          ? "bg-[var(--acc)] font-semibold text-white"
          : "border border-[var(--pm-ui-border)] bg-white font-medium text-[var(--ink)]"
      }`}
    >
      {children}
    </button>
...
function PgHandoffCard({ plan }: { plan: OwnerPlan }) {
  return (
    <div className="rounded-[10px] border border-[var(--pm-ui-border)] bg-white p-4">
      <div className="rounded-[8px] border border-[var(--pm-ui-border)] bg-white px-4 py-4">
        <div className="flex items-start gap-3">
          <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-[8px] border border-[var(--pm-ui-border)] bg-[var(--acc-tint)] text-[var(--acc)]">
            <CreditCard className="h-[18px] w-[18px]" />
          </span>
          <div className="min-w-0">
            <p className="text-[15px] font-medium tracking-[-0.02em] text-[var(--ink)]">보안창에서 카드 등록이 진행돼요</p>
            <p className="mt-1 text-[13px] leading-[1.6] text-[var(--mid)]">
...
export function OwnerBillingProcessPreview() {
  ...
  return (
    <div className="owner-font pm-owner-web min-h-screen bg-[var(--bg)] px-4 py-8 text-[var(--ink)]">
      <div className="mx-auto max-w-[1120px]">
        <header className="mb-6">
          <p className="text-[13px] font-medium tracking-[-0.02em] text-[var(--acc)]">카드사 제출용</p>
          <h1 className="mt-2 text-[30px] font-semibold tracking-[-0.05em] text-[var(--ink)]">정기결제 프로세스</h1>
          <p className="mt-3 text-[14px] leading-[1.6] text-[var(--mid)]">
        ...
        <div className="space-y-6">
              ...
              >
                <div className="rounded-[10px] border border-[var(--pm-ui-border)] bg-white p-4">
                  <button
                    ...
                    className="inline-flex h-[46px] items-center justify-center rounded-[8px] border border-[var(--pm-ui-border)] bg-white px-4 text-[14px] font-medium text-[var(--ink)]"
                  >
                    결제수단 선택 시트 보기
                  </button>
                </div>
          ...
          >
            <div className="rounded-[10px] border border-[var(--pm-ui-border)] bg-white p-5">
              <div className="rounded-[8px] border border-[var(--pm-ui-border)] bg-white px-4 py-4">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <WalletCards className="h-5 w-5 text-[var(--acc)]" />
                    <div>
                      <p className="text-[15px] font-medium text-[var(--ink)]">현재 등록 카드</p>
                      <p className="mt-1 text-[13px] text-[var(--mid)]">하나카드 ·•••• 1234</p>
                    </div>
                  </div>
                  <button type="button" className="text-[14px] font-medium text-[var(--acc)]">
                    카드 변경
                  </button>
                </div>
              </div>
          ...
          >
            <div className="rounded-[10px] border border-[var(--pm-ui-border)] bg-white p-5">
              <div className="rounded-[8px] border border-[var(--pm-ui-border)] border-l-[3px] border-l-[#b98121] bg-white px-4 py-4">
                <div className="flex items-start gap-3">
                  <Ban className="mt-0.5 h-5 w-5 shrink-0 text-[#b98121]" />
                  <div className="space-y-1.5 text-[14px] leading-[1.55] text-[var(--mid)]">
                    <p className="font-medium text-[var(--ink)]">정기결제를 해지할 수 있어요</p>
          ...
          >
            <div className="rounded-[10px] border border-[var(--pm-ui-border)] bg-white p-5">
              <div className="rounded-[8px] border border-[var(--pm-ui-border)] border-l-[3px] border-l-[#a04455] bg-white px-4 py-4">
                <div className="flex items-start gap-3">
                  <AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-[#a04455]" />
                  <div className="space-y-1.5 text-[14px] leading-[1.55] text-[var(--mid)]">
                    <p className="font-medium text-[var(--ink)]">이번 결제를 완료하지 못했어요</p>
          ...
          <div className="rounded-[10px] border border-[var(--pm-ui-border)] border-l-[3px] border-l-[#2563eb] bg-white px-5 py-4">
            <div className="flex items-start gap-3">
              <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-[#2563eb]" />
              <div className="text-[13px] leading-[1.6] text-[var(--mid)]">
                <p className="font-medium text-[var(--ink)]">심사자료 기재 안내</p>
                <p className="mt-1">
                  카드번호 전체와 CVC는 PG 보안창에서만 입력되며, {PETMANAGER_SERVICE_NAME}는 빌링용
                  마스킹 카드 정보만 관리하는 구조로 안내합니다.
                </p>
              </div>
            </div>
          </div>
