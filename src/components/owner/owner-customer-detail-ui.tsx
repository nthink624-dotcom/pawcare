...
export function Avatar({ seed }: { seed: string }) {
  return <div className="flex size-11 items-center justify-center rounded-full border border-[#dce4ef] bg-[#f8fafc] text-lg shadow-[0_2px_8px_rgba(15,23,42,0.05)]">{seed}</div>;
}
...
export function NotificationHistoryRow({ notification, pet }: { notification: BootstrapPayload["notifications"][number]; pet: Pet | null }) {
  ...
  const isDelivered = notification.status === "sent" || notification.status === "mocked" || Boolean(notification.sent_at || notification.provider_message_id);
  const statusLabel =
    isDelivered
      ? "발송 완료"
