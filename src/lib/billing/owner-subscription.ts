...
export function normalizeOwnerSubscriptionMetadata(
  ...
  const trialStartedAt = typeof metadata?.trial_started_at === "string" ? metadata.trial_started_at : baseCreatedAt;
  const trialEndsAt = typeof metadata?.trial_ends_at === "string" ? metadata.trial_ends_at : addDaysIso(trialStartedAt, OWNER_TRIAL_DAYS);
  ...
  const autoRenewPlanCode =
    metadata?.auto_renew_plan_code === "quarterly" ||
    metadata?.auto_renew_plan_code === "halfyearly" ||
    metadata?.auto_renew_plan_code === "yearly"
      ? metadata.auto_renew_plan_code
      : "monthly";
  const currentPlanCode =
    metadata?.current_plan_code === "quarterly" ||
    metadata?.current_plan_code === "halfyearly" ||
    metadata?.current_plan_code === "yearly"
      ? metadata.current_plan_code
      : autoRenewPlanCode;
  const paymentMethodExists = metadata?.payment_method_exists === true;
  const paymentMethodLabel = typeof metadata?.payment_method_label === "string" ? metadata.payment_method_label : null;
  ...
  const lastPaymentFailedAt = typeof metadata?.last_payment_failed_at === "string" ? metadata.last_payment_failed_at : null;
  const lastPaymentId = typeof metadata?.last_payment_id === "string" ? metadata.last_payment_id : null;
  const currentPeriodStartedAt = typeof metadata?.current_period_started_at === "string" ? metadata.current_period_started_at : null;
  const currentPeriodEndsAt = typeof metadata?.current_period_ends_at === "string" ? metadata.current_period_ends_at : null;
  const nextBillingAt = typeof metadata?.next_billing_at === "string" ? metadata.next_billing_at : currentPeriodEndsAt;
  ...
  if (storedStatus === "past_due") {
    ...
    if (daysUntilPeriodEnds !== null && daysUntilPeriodEnds <= 0) {
      status = "expired";
    } else {
      status = "active";
    }
  } else if ((storedStatus === "trialing" || storedStatus === "trial_will_end") && daysUntilTrialEnds <= 3 && daysUntilTrialEnds > 0) {
  ...
  const currentPlan = ownerPlans.find((plan) => plan.code === currentPlanCode) ?? ownerPlans[0];
  const autoRenewPlan = ownerPlans.find((plan) => plan.code === autoRenewPlanCode) ?? ownerPlans[0];
