...
export function getSupabaseCookieOptions(options?: { secure?: boolean; hostname?: string }) {
  const browserSecure = typeof window !== "undefined" ? window.location.protocol === "https:" : undefined;
  const secure = options?.secure ?? browserSecure ?? getSupabaseRuntimeStage() === "production";
  const productionOptions = shouldUseProductionCookieDomain(options?.hostname)
    ? {
