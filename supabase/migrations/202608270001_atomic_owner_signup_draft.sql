-- DRAFT ONLY: Development signup integration v0.2.
-- Do not apply to Production without Security P0 gate approval and a reviewed rollback runbook.

create table if not exists public.signup_idempotency_requests (
  signup_request_id uuid primary key,
  payload_hash text not null check (length(payload_hash) = 64),
  status text not null check (status in ('processing', 'completed', 'compensation_pending', 'failed_compensated')),
...
alter table public.signup_idempotency_requests enable row level security;
revoke all on table public.signup_idempotency_requests from public, anon, authenticated;
grant select, insert, update on table public.signup_idempotency_requests to service_role;
...
begin
  ...
  if v_shop_id is null or jsonb_typeof(p_services) <> 'array' or jsonb_array_length(p_services) < 1 then
