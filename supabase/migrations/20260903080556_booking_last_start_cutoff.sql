-- A null value preserves the legacy completion-within-window rule.  A selected
-- offset switches only the latest *start* boundary; break, closure, overlap,
-- capacity, and staff eligibility remain independently enforced.
alter table public.shops
  add column if not exists last_booking_start_offset_minutes integer;

alter table public.shops
  drop constraint if exists shops_last_booking_start_offset_minutes_check,
  add constraint shops_last_booking_start_offset_minutes_check
    check (last_booking_start_offset_minutes is null or last_booking_start_offset_minutes in (0, 30, 60, 90, 120));
...
begin
  ...
  if nullif(btrim(v_shop.booking_available_start_time), '') is null
    ...
    raise exception using errcode = '22023', message = 'booking shop window is invalid';
  end if;
  v_shop_start := greatest(
    (v_hours ->> 'open')::time,
    v_shop.booking_available_start_time::time
  );
  v_shop_end := least(
    (v_hours ->> 'close')::time,
    v_shop.booking_available_end_time::time
  );
  if v_shop_start >= v_shop_end or new.appointment_time < v_shop_start then
    raise exception using errcode = '22023', message = 'booking start is outside the shop window';
  end if;
  ...
  if new.staff_id is not null then
    ...
    v_effective_end := least(v_shop_end, v_staff_end);
  else
    v_effective_end := v_shop_end;
  end if;

  if v_shop.last_booking_start_offset_minutes is null then
    if new.appointment_time + v_duration > v_effective_end then
      raise exception using errcode = '22023', message = 'booking must finish within the working window';
    end if;
  else
    v_cutoff := v_effective_end - make_interval(mins => v_shop.last_booking_start_offset_minutes);
    if new.appointment_time > v_cutoff then
      raise exception using errcode = '22023', message = 'booking start is after the last booking cutoff';
    end if;
  end if;
...
drop trigger if exists appointments_assert_booking_start_cutoff on public.appointments;
create trigger appointments_assert_booking_start_cutoff
before insert or update of appointment_date, appointment_time, staff_id, status, start_at, end_at
on public.appointments
for each row
execute function private.assert_appointment_booking_start_cutoff();

alter table public.appointments
  enable trigger appointments_assert_booking_start_cutoff;
...
begin
  if not exists (
    ...
    raise exception using errcode = '55000', message = 'booking cutoff trigger is not enabled or bound to its guard';
  end if;
end;
$$;
