-- Signup-only v6 prerequisites; independent of the broad billing draft.
-- No payment/subscription triggers or updates to historical subscription rows.
...
alter table public.owner_subscriptions
  add constraint owner_subscriptions_single_monthly_v1_contract_check check (
    not (current_plan_code = 'single_monthly_v1' or featured_plan_code = 'single_monthly_v1' or auto_renew_plan_code = 'single_monthly_v1')
    or ((product_version is not distinct from '2026-08-v1' or product_version is not distinct from '2026-09-v1')
...
begin
  if p_owner_user_id is null
     ...
     or p_consent_document_version is distinct from '2026-09-14-v1'
     or p_consented is null
     or p_recorded_at is null
     or p_collection_channel is null
     or p_collection_channel not in ('owner_signup', 'owner_settings')
     or p_collection_method is null
  ...
  if v_event.event_id is null and p_signup_request_id is not null then
    ...
    -- Historical signup retry cannot restore withdrawn consent.
...
begin
  if p_marketing_consent is null
     or p_marketing_consent_document_version is distinct from '2026-09-14-v1'
     or p_marketing_consent_recorded_at is null
     or p_shop_identity_key_version is null
     or p_shop_identity_key_version !~ '^v[0-9]+$'
     or p_shop_identity_key is null
