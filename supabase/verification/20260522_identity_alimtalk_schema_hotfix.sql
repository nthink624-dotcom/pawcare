-- Title: 2026-05-22 본인확인/알림톡 크레딧 스키마 보강
-- Target: development Supabase first, then production Supabase after smoke check.
-- Purpose:
--   1. 본인확인 요청/완료/소비 이력을 DB에 저장합니다.
--   2. 매장별 알림톡 무료/유료 잔여 수량과 차감/환불 장부를 저장합니다.
-- Apply in Supabase SQL Editor when these tables/views are missing:
--   public.owner_identity_verifications
--   public.shop_alimtalk_credit_balances
--   public.shop_alimtalk_credit_events
--   public.shop_alimtalk_credit_summaries

create table if not exists public.owner_identity_verifications (
...
create unique index if not exists owner_identity_verifications_provider_identity_verification_id_key
  on public.owner_identity_verifications(provider_identity_verification_id)
  where provider_identity_verification_id is not null;

create index if not exists owner_identity_verifications_status_idx
  on public.owner_identity_verifications(status, verified_expires_at desc, challenge_expires_at desc);

create index if not exists owner_identity_verifications_purpose_idx
  on public.owner_identity_verifications(purpose, created_at desc);
