...
test("daily schedule keeps staff identity, selection, and current-time colors in separate roles", () => {
  ...
  assert.match(staffColors, /background: "#FBECEF"/);
  assert.match(staffColors, /background: "#FDF0E8"/);
  assert.match(staffColors, /background: "#FFFBE3"/);
  assert.match(staffColors, /background: "#EAF6EF"/);
  assert.match(staffColors, /background: "#EAF4FC"/);
  assert.match(staffColors, /background: "#EDF0F7"/);
  assert.match(staffColors, /background: "#F0EAF7"/);
  assert.match(staffColors, /background: "#F5EDE7"/);
  assert.match(staffColors, /background: "#E6F6F7"/);
  assert.match(staffColors, /background: "#F7F2E9"/);
  ...
  assert.match(header, /h-\[60px\][^"\n]*bg-white/);
  assert.doesNotMatch(header, /backgroundColor: headerTone\.background/);
  assert.match(header, /data-schedule-staff-header-accent="true"/);
  assert.match(header, /left-4 h-\[2px\] w-12/);
  assert.match(header, /backgroundColor: headerTone\.selectedBackground/);
  assert.match(header, /className="h-11 w-11 border-\[#e8edf3\]/);
  ...
  assert.doesNotMatch(grid, /getScheduleStaffIdentityTone|data-booking-staff-identity|--pm-booking-status-edge/);
  assert.match(grid, /getAppointmentIdentityTone\(booking\.petId \?\? booking\.pet \?\? booking\.id\)/);
  ...
  assert.match(grid, /identityTone\.text/);
  assert.match(grid, /inline-flex shrink-0 items-center rounded-full border px-1\.5 text-\[12px\] font-medium leading-\[18px\]/);
  assert.match(grid, /data-booking-density=\{density\}/);
  assert.match(staffManagement, /backgroundColor: tone\.selectedBackground, borderColor: tone\.selectedBackground/);
  assert.doesNotMatch(staffManagement, /backgroundColor: tone\.background, borderColor: tone\.border/);
