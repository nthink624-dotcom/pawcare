...
test("accepted route cores bypass only their redundant inner wrapper", async () => {
  const [calendar, profitability, bookingLink, shopInfo, alerts] = await Promise.all([
  ...
  assert.match(calendarRoot, /<section className="flex h-full min-h-0 min-w-0 flex-col overflow-y-auto bg-white sm:overflow-hidden">/);
  ...
  assert.match(calendarRoot, /min-w-\[980px\] grid-cols-7/, "calendar month grid keeps an intentional local horizontal scroll surface");
  assert.match(calendarRoot, /min-w-0 overflow-x-auto/, "calendar month grid local scroll must remain reachable");
  ...
  assert.match(shopInfoRoot, /<div className="grid h-full min-h-0 gap-2 xl:grid-cols-\[minmax\(0,1fr\)_320px\]">/);
  ...
  assert.match(shopInfo, /import \{ CustomerPagePhonePreview \}/, "shop info must retain the customer phone preview import");
  ...
  assert.match(shopInfo, /const customerPreviewShop = useMemo/, "shop info preview must derive the current form state");
  assert.match(shopInfo, /<CustomerPagePhonePreview\s+shop=\{customerPreviewShop\}\s+services=\{previewServices\}\s+staffMembers=\{staffMembers\}\s+ownerProfile=\{ownerProfile\}/, "shop info must render the restored phone preview on xl");
  ...
  assert.match(alertsRoot, /<section className="min-w-0">\s*<div className="grid min-w-0 gap-5 xl:grid-cols-\[minmax\(0,1fr\)_380px\]">/);
  ...
  assert.match(alerts, /w-full items-center whitespace-normal break-words[\s\S]*sm:w-auto sm:whitespace-nowrap/, "long Korean alert labels must remain visible");
  assert.match(alerts, /h-11 w-\[60px\] focus-visible:outline-\[#2563eb\][\s\S]*thumbClassName="h-8 w-8"/, "alert switches must expose 44px clear-blue keyboard targets");
});
