...
test("booking link preserves URL, copy actions, customer opening, and owner route wiring", () => {
  ...
  assert.match(bookingLink, /href=\{bookingUrl\}[\s\S]{0,180}target="_blank"[\s\S]{0,220}고객 화면 열기/);
...
test("booking link reflows at narrow widths without a page-level horizontal scroller", () => {
  ...
  assert.match(bookingLink, /grid min-w-0 items-start gap-5 xl:grid-cols-2 xl:gap-0/);
  assert.match(bookingLink, /border-t border-\[#e8edf3\] pt-5 xl:border-l xl:border-t-0 xl:pl-4 xl:pt-0/);
