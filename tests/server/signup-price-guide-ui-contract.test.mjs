...
test("owner setup uses an accessible guide and a non-dismissible blocking modal over inert operations", async () => {
  ...
  assert.match(guide, /className="flex min-w-0 flex-1 basis-\[160px\] items-center gap-1"/);
  assert.match(guide, /data-testid="owner-initial-setup-title-actions"/);
  assert.match(guide, /className="flex min-w-0 w-full flex-wrap items-center justify-end gap-1 sm:w-auto sm:flex-1"/);
  assert.doesNotMatch(guide, /data-testid="owner-initial-setup-action-row"/);
  assert.doesNotMatch(guide, /className="mt-3 flex min-h-11 min-w-0 justify-end empty:hidden"/);
...
test("service setup separates new-shop choice, inline direct matrix, photo review, and saved management", async () => {
  ...
  assert.match(choice, /min-h-\[100px\]/);
  assert.match(choice, /sm:grid-cols-2/);
  assert.match(choice, /onClick=\{\(\) => onSelect\(mode\)\}/);
  assert.match(choice, /사진 한 장으로 요금표를 빠르게 시작할 수 있어요\./);
  assert.match(choice, /서비스와 가격을 직접 입력할 수 있어요\./);
  ...
  assert.match(directMatrix, /<Plus className="h-4 w-4" aria-hidden="true" \/>몸무게/);
  ...
  assert.match(directMatrix, /addDirectPriceGuideService/);
  assert.match(directMatrix, /removeDirectPriceGuideService/);
  assert.match(directMatrix, /updateDirectPriceGuideService/);
  assert.match(directMatrix, /group\.serviceNames\.length \* 210/);
  assert.match(directMatrix, /BreedManagementDialog/);
...
test("DB-free owner setup preview renders all three fixture forms and real save callbacks", async () => {
  ...
  assert.match(manual, /<PriceGuideStructuredReviewTable[\s\S]*document=\{draft\}[\s\S]*onEdit=/);
  assert.match(manual, /photoReviewMode=\{!manualMatrixMode\}/);
  assert.doesNotMatch(directMatrix, /바꿀 칸을 누르면 그 자리에서 입력할 수 있어요\. 저장 버튼을 누르기 전에는 반영되지 않습니다\./);
  assert.match(directMatrix, /text-\[20px\] font-semibold leading-7/);
